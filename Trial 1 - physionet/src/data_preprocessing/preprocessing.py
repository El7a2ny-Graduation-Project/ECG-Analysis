...
class PhysioNetDataset:
    def __init__(self, data_dir=None, target_fs=300, window_size=3000, stride=2000, save_path=None):
    ...
    def preprocess_record(self, record_name):
        """Load, normalize, and segment a single ECG record."""
        ecg_signal = self.load_ecg(record_name)  # Load raw signal
        ecg_signal = self.normalize_signal(ecg_signal)  # Normalize
        segments = self.segment_signal(ecg_signal)  # Segment into windows
